...
raw_table['Date'] = pd.to_datetime(raw_table['Date'], errors='coerce')
raw_table['DoB'] = pd.to_datetime(raw_table['DoB'], errors='coerce')
...
# -----------------------------
# Relative percentages (absolute method with max=20)
max_scores_by_pos = {pos: sum(weight * 20 for attr, weight in coeffs.items()) for pos, coeffs in position_coeffs.items()}

for pos in all_unique_positions:
    key = get_key_for_pos(pos)
    pos_col = f'{pos}_rating'
    if key and key in max_scores_by_pos:
        max_val = max_scores_by_pos[key]
        if max_val > 0:
...
df['phys_score'] = df[['Pac', 'Acc', 'Jum']].sum(axis=1)
max_phys = 60
df['phys_rating'] = (df['phys_score'] / max_phys) * 100


# List all unique attributes from position_coeffs
key_attrs = set(attr for coeffs in position_coeffs.values() for attr in coeffs.keys())#

key_attrs_in_df = [attr for attr in key_attrs if attr in df.columns]


# Compute key_score as % of max (20 per attribute)
df['key_score'] = df[list(key_attrs_in_df)].sum(axis=1) / (len(key_attrs_in_df) * 20) * 100
df['key_rating'] = df['key_score'].round(2)

# -----------------------------
# Attach first appearance ratings
df['Date'] = pd.to_datetime(df['Date'])

# Find first export per player
first_ratings = (
    df.sort_values('Date')
      .groupby('UID')
      .first()
      .reset_index()[['UID', 'key_rating', 'phys_rating']]
)

# Rename to avoid confusion
first_ratings = first_ratings.rename(
    columns={'key_rating': 'first_key_rating', 'phys_rating': 'first_phys_rating'}
)

# Join back to main df
df = df.merge(first_ratings, on='UID', how='left')

df['key_growth'] = (df['key_rating'] - df['first_key_rating']).round(2)
df['phys_growth'] = (df['phys_rating'] - df['first_phys_rating']).round(2)

# -----------------------------
# Season-based Age Calculation
def get_season_jan1(export_date):
    """Return Jan 1st reference date for the season in which export_date falls."""
    if export_date.month >= 7:  # season starts July this year
        return pd.Timestamp(year=export_date.year + 1, month=1, day=1)
    else:  # season is still in last year's season
        return pd.Timestamp(year=export_date.year, month=1, day=1)

df['season_jan1'] = df['Date'].apply(get_season_jan1)

# Age as int at Jan 1st of that season
df['Age_int'] = ((df['season_jan1'] - df['DoB']).dt.days // 365).astype('Int64')

df = df.drop(columns=['season_jan1'])


# -----------------------------
# Visualization DataFrame assembly
df['Positions_filter'] = df['Positions'].apply(lambda x: ', '.join(x) + ', All')
df['Positions'] = df['Positions'].apply(lambda x: ', '.join(x))

viz_df = df[['Name','Age_int','Positions','Nat','Positions_filter', 'phys_rating','phys_growth', 'key_rating','key_growth'] +
            percentage_columns +
            list(all_unique_positions) +   # keep for toggle logic
            ['is_latest_export','Date']].round(2)
viz_df['Age_int'] = viz_df['Age_int'].apply(np.floor).astype(int)
...
# Display subset
display_positions = ['GK','DC','DMC','DMR','DML','AMC','ST']
rating_columns_display = [f'{pos}_%' for pos in display_positions if f'{pos}_%' in viz_df.columns]

# Max rating only from display subset
viz_df['Max_Rating'] = viz_df[rating_columns_display].max(axis=1)
...
def update_datatable(selected_age_range, selected_positions,  name_filter, show_all_toggle):
    ...
    # Columns to display
    columns_to_show = ['Name','Age_int','Positions','Nat'] + rating_columns_display + ['Max_Rating','phys_rating','phys_growth','key_rating','key_growth']
    ...
    dbc.Row(
        dbc.Col(
            dash_table.DataTable(
                ...
                # Conditional formatting only for display subset
                style_data_conditional=[
                    {
                        'if': {'filter_query': f'{{{col}}} >= 70', 'column_id': col},
                        'backgroundColor': '#c6efce', 'color': 'black'
                    } for col in rating_columns_display
                ] + [
                    {
                        'if': {'filter_query': f'{{{col}}} >= 30 && {{{col}}} < 70', 'column_id': col},
                        'backgroundColor': '#ffeb9c', 'color': 'black'
                    } for col in rating_columns_display
                ] + [
                    {
                        'if': {'filter_query': f'{{{col}}} < 30 && {{{col}}} > 0', 'column_id': col},
                        'backgroundColor': '#f2dcdb', 'color': 'black'
                    } for col in rating_columns_display
                ] + [
                    {
                        'if': {'filter_query': f'{{{col}}} = 0', 'column_id': col},
                        'backgroundColor': 'white', 'color': 'lightgrey'
                    } for col in rating_columns_display
                ]
            )
        ),
